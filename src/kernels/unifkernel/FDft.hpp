 ...
 *
 * The @class FDft is templatized with the input value type (FReal or std::complex<FReal>),
...
class FDft
{
    ...
    std::complex<FReal>* dataF_; //< data in Fourier space
...
private:
    ...
    void initDFT()
    {
        ...
        dataF_ = new std::complex<FReal>[size_];
...
public:
    ...
    void applyDFT(const FReal* sampledData,
                  std::complex<FReal>* transformedData) const
    {
        // read sampled data
        ...
        for(unsigned int r=0; r<size_; ++r)
            for(unsigned int s=0; s<size_; ++s){
                dataF_[r] += std::complex<FReal>(data_[s]*cosRS_[r*size_+s],
    ...
    // Complexe valued DFT
    void applyDFT(const std::complex<FReal>* sampledData,
                  std::complex<FReal>* transformedData) const
    {
        // read sampled data
        FBlas::c_setzero(size_,reinterpret_cast<FReal*>(dataF_));
        ...
        for(unsigned int r=0; r<size_; ++r)
            for(unsigned int s=0; s<size_; ++s){
                dataF_[r] += std::complex<FReal>(data_[s].getReal()*cosRS_[r*size_+s]
    ...
    /// Backward DFT
    // Real valued IDFT
    void applyIDFT(const std::complex<FReal>* transformedData,
    ...
    void applyIDFT(const std::complex<FReal>* transformedData,
                   std::complex<FReal>* sampledData) const
    {
        // read transformed data
        ...
        for(unsigned int r=0; r<size_; ++r){
            for(unsigned int s=0; s<size_; ++s){
                data_[r] += std::complex<FReal>(dataF_[s].getReal()*cosRS_[r*size_+s]
                        - dataF_[s].getImag()*sinRS_[r*size_+s],
...
protected:
    ...
    static void PlusEqual(std::complex<double> dest[], const fftw_complex src[], const int nbElements){
        const double* ptrSrc = reinterpret_cast<const double*>(src);
        for(int idxVal = 0 ; idxVal < nbElements ; ++idxVal){
            dest[idxVal] += std::complex<double>(ptrSrc[2*idxVal], ptrSrc[2*idxVal+1]);
        }
    }
    ...
    static void Equal(std::complex<double> dest[], const fftw_complex src[], const int nbElements){
        const double* ptrSrc = reinterpret_cast<const double*>(src);
        for(int idxVal = 0 ; idxVal < nbElements ; ++idxVal){
            dest[idxVal] = std::complex<double>(ptrSrc[2*idxVal], ptrSrc[2*idxVal+1]);
        }
    ...
    static void PlusEqual(std::complex<float> dest[], const fftwf_complex src[], const int nbElements){
        const float* ptrSrc = reinterpret_cast<const float*>(src);
        for(int idxVal = 0 ; idxVal < nbElements ; ++idxVal){
            dest[idxVal] += std::complex<float>(ptrSrc[2*idxVal], ptrSrc[2*idxVal+1]);
        }
    }
    ...
    static void Equal(std::complex<float> dest[], const fftwf_complex src[], const int nbElements){
        const float* ptrSrc = reinterpret_cast<const float*>(src);
        for(int idxVal = 0 ; idxVal < nbElements ; ++idxVal){
            dest[idxVal] = std::complex<float>(ptrSrc[2*idxVal], ptrSrc[2*idxVal+1]);
        }
...
public:
    ...
    */
    ...
    void normalize(std::complex<double>* resultSignal) const {
        const double realNbPoints = static_cast<double>(nbPoints);
        for(int idxVal = 0 ; idxVal < nbPoints ; ++idxVal){
            resultSignal[idxVal] /= realNbPoints;
        }
    ...
    void normalize(std::complex<float>* resultSignal) const {
        const float realNbPoints = static_cast<float>(nbPoints);
        for(int idxVal = 0 ; idxVal < nbPoints ; ++idxVal){
            resultSignal[idxVal] /= realNbPoints;
        }
...
//////////////////////////////////////////////////////////////////////////////
template <int DIM>
class FFftw <double, std::complex<double>, DIM> : public FFftwCore<double, std::complex<double>, double, fftw_complex, fftw_plan, DIM>{
    typedef FFftwCore<double, std::complex<double>, double, fftw_complex, fftw_plan, DIM> ParentClass;
public:
    using ParentClass::ParentClass;
};
template <int DIM>
class FFftw <std::complex<double>, std::complex<double>, DIM> : public FFftwCore<std::complex<double>, std::complex<double>, fftw_complex, fftw_complex, fftw_plan, DIM>{
    typedef FFftwCore<std::complex<double>, std::complex<double>, fftw_complex, fftw_complex, fftw_plan, DIM> ParentClass;
public:
    using ParentClass::ParentClass;
};
//////////////////////////////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////////////////////////////
template <int DIM>
class FFftw <float, std::complex<float>, DIM> : public FFftwCore<float, std::complex<float>, float, fftwf_complex, fftwf_plan, DIM>{
    typedef FFftwCore<float, std::complex<float>, float, fftwf_complex, fftwf_plan, DIM> ParentClass;
public:
    using ParentClass::ParentClass;
};
template <int DIM>
class FFftw <std::complex<float>, std::complex<float>, DIM> : public FFftwCore<std::complex<float>, std::complex<float>, fftwf_complex, fftwf_complex, fftwf_plan, DIM>{
    typedef FFftwCore<std::complex<float>, std::complex<float>, fftwf_complex, fftwf_complex, fftwf_plan, DIM> ParentClass;
public:
    using ParentClass::ParentClass;
};
