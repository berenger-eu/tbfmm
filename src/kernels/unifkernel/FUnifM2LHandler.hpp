...
static void Compute(const MatrixKernelClass *const MatrixKernel, const FReal CellWidth, std::complex<FReal>* &FC, const int SeparationCriterion = 1)
{
    const int Dim = 3;
    ...
    FFftw<FReal,std::complex<FReal>,dimfft> Dft(rc);
    ...
    // allocate M2L
    FC = new std::complex<FReal>[343 * opt_rc];
...
class FUnifM2LHandler<FReal, ORDER,HOMOGENEOUS>
{
    ...
    std::shared_ptr< std::complex<FReal>[] > FC;
    ...
    typedef FFftw<FReal,std::complex<FReal>,dimfft> DftClass; // Fast Discrete Fourier Transformator
    DftClass Dft;
...
public:
    ...
    void ComputeAndSet(const MatrixKernelClass *const MatrixKernel)
    {
        ...
        std::cout << "Compute and set M2L operators ("<< long(sizeM2L/**1e-6*/) <<" B) in "
                  << time.stopAndGetElapsed() << "sec."   << std::endl;
    }

    unsigned long long getMemory() const {
        return 343*opt_rc*sizeof(std::complex<FReal>);
    }        
    ...
    */
    void unapplyZeroPaddingAndDFT(const std::complex<FReal> *const FX, FReal *const x) const
    { 
        FReal Px[rc];
        FBlas::setzero(rc,Px);
    ...
    void applyFC(const unsigned int idx, const unsigned int, const FReal scale,
                 const std::complex<FReal> *const FY, std::complex<FReal> *const FX) const
    {
     ...
     */
    void applyZeroPaddingAndDFT(FReal *const y, std::complex<FReal> *const FY) const
    {
        FReal Py[rc];
        FBlas::setzero(rc,Py);
        // Apply Zero Padding
...
class FUnifM2LHandler<FReal,ORDER,NON_HOMOGENEOUS>
{
    ...
    /// DFT specific
    static const int dimfft = 1; // unidim FFT since fully circulant embedding
    typedef FFftw<FReal,std::complex<FReal>,dimfft> DftClass; // Fast real-valued Discrete Fourier Transformator
    DftClass Dft;
    const unsigned int opt_rc; // specific to real valued kernel
...
public:
          ...
          Dft(rc), opt_rc(rc/2+1), LeafLevelSeparationCriterion(inLeafLevelSeparationCriterion)
    {
        ...
        // init M2L operators
        FC = new std::complex<FReal>*[TreeHeight];
        FC[0]       = NULL; FC[1]       = NULL;
    ...
    void ComputeAndSet(const MatrixKernelClass *const MatrixKernel)
    {
        ...
        // Compute memory usage
        unsigned long sizeM2L = (TreeHeight-2)*343*opt_rc*sizeof(std::complex<FReal>);
    ...
    unsigned long long getMemory() const {
        return (TreeHeight-2)*343*opt_rc*sizeof(std::complex<FReal>);
    }   
    ...
    */
    ...
    const std::complex<FReal>& getFc(const int i, const int j) const{
