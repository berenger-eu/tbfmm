...
class FHarmonic  {
    const FReal PI = 3.14159265358979323846264338327950288419716939937510582097494459230781640628620899863;
    const FReal PIDiv2 = PI/2;
    ...
    std::complex<FReal>* harmonic;//< Harmonic Result
    std::complex<FReal>* cosSin;  //< Cos/Sin precomputed values
    FReal*     legendre;//< Legendre results

    std::complex<FReal>* thetaDerivatedResult; //< the theta derivated result
    ...
    /** Allocate and init */
    void allocAndInit(){
        harmonic = new std::complex<FReal>[expSize];
        cosSin   = new std::complex<FReal>[devP + 1];
        legendre = new FReal[expSize];
        thetaDerivatedResult = new std::complex<FReal>[expSize];
    ...
    void computeCosSin(const FReal inPhi, const FReal piArray[4]){
        for(int l = 0 ; l <= devP ; ++l){
            ...
            cosSin[l].setReal( std::sin(angle + PIDiv2) );  // add Pi/2 for the cosine

            cosSin[l].setImag( std::sin(angle) );
        }
    }
...
public:
    ...
    std::complex<FReal>* result(){
        return harmonic;
    }

    const std::complex<FReal>* result() const {
        return harmonic;
    }

    std::complex<FReal>& result(const int index){
        return harmonic[index];
    }

    const std::complex<FReal>& result(const int index) const{
        return harmonic[index];
    }

    std::complex<FReal>* resultThetaDerivated(){
        return thetaDerivatedResult;
    }

    const std::complex<FReal>* resultThetaDerivated() const {
        return thetaDerivatedResult;
    }

    std::complex<FReal>& resultThetaDerivated(const int index){
        return thetaDerivatedResult[index];
    }

    const std::complex<FReal>& resultThetaDerivated(const int index) const{
        return thetaDerivatedResult[index];
    }
    ...
    void computeInner(const FSpherical<FReal>& inSphere){
        // For i^|m|
        const FReal PiArrayInner[4] = {0, PIDiv2, PI, -PIDiv2};
        //
    ...
    */
    void computeInnerOnZaxis(const FReal& inRadius,FReal* Inner){
        // Not used : const FReal PiArrayInner[4] = {0, PIDiv2, PI, -PIDiv2};
        //
        int   index_l = 0   ;
        FReal r_pow_l = 1.0 ;
    ...
    void computeOuter(const FSpherical<FReal>& inSphere){
        const FReal PiArrayOuter[4] = {0, -PIDiv2, PI, PIDiv2};
