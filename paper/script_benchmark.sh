...
function execute(){
    ...
    for nb_particles_height in "1000000:5" "10000000:6" ; do
        ...
        for nb_threads in 1 2 4 8 16 32 ; do
            echo "$nb_threads"
            result=$(TBFMM_BLOCK_SIZE=128 OMP_PROC_BIND=true OMP_NUM_THREADS=$nb_threads ./testUnifKernel -nb $nb_particles -th $height -nc)
            ...
            line="$nb_particles,$height,$nb_threads,128,$exec_time,$build_time,$algo_name"
            echo "$line"
            all_lines="$all_lines\n$line"
        done
    done
