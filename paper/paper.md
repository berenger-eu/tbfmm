...
`TBFMM` was designed to be easy to customize thanks to `C++` templates and fine control of the `C++` classes inter-dependencies.
...
Because it is a fundamental building block for many types of simulation, the FMM parallelization has already been investigated.
...
This is because some stages of the FMM have a small degree of parallelism (for instance at the top of the tree), while others have a high degree with a significant workload available from the early beginning of each iteration (for instance the `P2P` in the direct pass).
...
The two main ideas behind this container are (1) to allocated and manage several cells of the same level together to control the granularity of the tasks, and (2) to store in a different memory block the symbolic data, the multipole data, and the local data.
This allows us to move each block anywhere on the memory nodes and declare the dependencies on each sub-part.
...
For the same reason, it is difficult for researchers in physics or applied mathematics to implement a complete FMM library and to optimize it for modern hardware, especially when their objective is to focus on approximation kernels.
...
Besides, it only works for 3D problems, where `TBFMM` can work for an arbitrary dimension.
These have been the main motivations to re-implement from scratch a lightweight FMM library that only supports task-based parallelization.
...
The algorithm has to be templatized to know the type of the kernel, and its  `execute`  method has to be templatized with the type of the tree. 
...
As we have clearly stated in the objectives, `TBFMM` is a tool for scientists from physics and applied mathematics to create new kernels.
...
With this aim, a user has to create a new kernel that respects an interface, as it is described the package documentation.
...
While `SPETABARU` supports commutative `write` access, `OpenMP` does it only from version 5 with the `mutexinout` data access.
`OpenMP` version 5 and is currently not fully supported by the compilers, but when a compiler that supports this access will be used with `TBFMM`, the `mutexinout` will be activated automatically.
...
In the first one, the regular algorithm and tree are used but when the algorithm needs cells outside the boundaries, it selects cells at the opposite of the simulation box.
...
Then, instead of stopping the FMM algorithm at level 2, we continue up to the root where the multipole part of the root represents the complete simulation box.
...
In Figure \autoref{fig:performance}, we provide the parallel efficiency of `TBFMM` for a set of particles randomly distributed in a square simulation box.
